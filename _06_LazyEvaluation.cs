...
namespace LinqFromScratch.V6
{
   ...
   public class _06_LazyEvaluation
   {
      ...
      public void _1_EagerEvaluation()
      {
         ...
         // This isn't a problem with such a trivially small sample, but our collections can
         // get MUCH bigger when running production code on real data.
      }
      ...
      public void _2_LazyEvaluation()
      {
         ...
         // And again, but without the debug/trace noise
         firstMatch =
             SampleData.EmployeeList
          ...
          Predicate<T> check)
      {
         ...
         foreach (var item in originalList)
         {
            ...
            if (check(item))
            {
               ...
               yield return item;
               // We can only do this in functions that return IEnumerables or IEnumerator
            }
            else
               Console.WriteLine("filtered item " + counter + " and check was false");
         }
