...
namespace LinqFromScratch
{
   ...
   public class _04_EnterGenerics
   {
      ...
      public void GenericSearching()
      {
         ...
         filteredEmployees = Filter<Employee>(SampleData.EmployeeList,
             x => x.Name.Contains("David"));
         Display.List(filteredEmployees, "Name contains David");

         // And now we can use it on different types. Hurrah.
         var filteredDepartments = Filter(SampleData.DepartmentList,
             x => !x.Name.Contains("front"));
      ...
      // The code changes basically involve swapping out mentions of Employee for the
