...
namespace LinqFromScratch
{
   ...
   {
      // We saw use of some delegates in the last page, so this starts with a second look at
      // the same sort of thing, before taking them in a different direction.

      // Here is a simple delegate that we'll be using. It takes a string and returns void.
      // It is a stupid, nonsensical example, we'd not want to do this in production code,
      // but it does show how we are passing around functions rather than variables.
      ...
      {
         // The previous examples were old-school C# anonymous methods, but the lambda syntax
         // simplified that a lot
         DisplayMessage delegated = delegate (string x) { Console.WriteLine(x); };
         DisplayMessage lambda_ed = x => Console.WriteLine(x);
         DelegateRunner(lambda_ed);
         // The function could be inlined, but separating it out makes the types more obvious
      }

      [TestMethod]
      public void _2_SimpleDelegateCallWithMethod()
      {
         // We don't always need to write lambdas, we can use a method too. Remember,
         // there's no brackets on the end, so we're still just passing the function,
         // not its result.
         DelegateRunner(BackwardsDisplayingMethod);
      }

      // The signature of the method just needs to match the signature of the delegate
      ...
      // it's processing, but if you're not writing a truly generic type then there is an
      // argument for being explicit, although Func & friends is idiomatic C# these days
      ...
      [TestMethod]
      public void _3_SimpleActionCall()
      {
      ...
      // Delegates are key to events, and events are key to a bunch of stuff in the .Net
      // framework. Here we have a simple class that raises events to signify a percentage
      // completion as that demos nicely in a test framework, but the most common events
      // are click handlers and the like which ultimately boil down to the same sort of
      // process.
      ...
      public void _4_EventExample()
      {
         ...
         // Register a couple of handlers, both with lambdas and methods. Multiple different
         // objects, potentially in different threads could register for the events
         importantProcess.PercentageComplete +=
             x => Console.WriteLine("Done " + x + " percent");
      ...
      // The "event" keyword gives us some syntactic sugar around the underlying delegate
      // mechanism, both for the subscriber code registering its handler, and the provider
      // code doing the same.
      [TestMethod]
      ...
      public class ManualImportantProcess
      {
         ...
         {
            PercentageComplete = (PercentageCompleteDelegate)Delegate.Combine(
                PercentageComplete, handler);
         }
